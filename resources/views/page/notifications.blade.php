    ...
    <div class="flex h-screen w-screen overflow-hidden">
        ...
        <div class="flex-1 flex flex-col overflow-hidden">
            ...
            <div class="flex-1 p-6 overflow-y-auto">
                ...
                @if(session('success'))
                <div class="mb-4 bg-green-50 border border-green-200 text-green-800 px-4 py-3 rounded-md" id="successMessage">
                    {{ session('success') }}
                </div>
                @endif

                @if(session('error'))
                <div class="mb-4 bg-red-50 border border-red-200 text-red-800 px-4 py-3 rounded-md" id="errorMessage">
                    {{ session('error') }}
                </div>
                @endif
                ...
                <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4 sm:gap-0">
                    ...
                    <div class="flex flex-wrap gap-2">
                        @if($unreadCount > 0)
                        <button id="markAllRead" class="inline-flex items-center px-4 py-2 border border-gray-300 shadow-sm text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500">
                            <i class="fas fa-check-circle mr-2"></i>
                            Tout marquer comme lu
                        </button>
                        @endif
                        <button class="inline-flex items-center px-4 py-2 border border-gray-300 shadow-sm text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500" onclick="window.location.reload()">
                    ...
                    <div class="notification-item {{ $notification->bg_class }} rounded-xl shadow-sm p-4 border transition-all duration-200"
                        ...
                        <div class="flex flex-col sm:flex-row">
                            <div class="mb-3 sm:mb-0 sm:mr-4">
                                <div class="h-12 w-12 rounded-full bg-white shadow-sm flex items-center justify-center {{ $notification->color_class }}">
                            ...
                            <div class="flex-1">
                                ...
                                <div class="flex flex-wrap gap-2">
                                    @if(!$notification->is_read)
                                    <button class="mark-as-read inline-flex items-center px-3 py-1.5 border border-gray-300 shadow-sm text-xs font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500"
                                            data-id="{{ $notification->id }}">
                                    ...
                                    <button class="delete-notification inline-flex items-center px-3 py-1.5 border border-red-300 shadow-sm text-xs font-medium rounded-md text-red-700 bg-white hover:bg-red-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500"
                                            data-id="{{ $notification->id }}">
        ...
        function markAsRead(notificationId, button) {
            ...
            .then(data => {
                if (data.success) {
                    ...
                    notificationItem.className = notificationItem.className.replace(/bg-\w+-50/, 'bg-white').replace(/border-\w+-100/, 'border-gray-200');
            ...
            .then(data => {
                ...
                if (data.count > 0) {
                    if (!badge) {
                        ...
                        newBadge.className = 'absolute top-0 right-0 h-4 w-4 bg-red-500 rounded-full border-2 border-white flex items-center justify-center';
                        newBadge.innerHTML = `<span class="text-xs text-white font-bold">${data.count > 9 ? '9+' : data.count}</span>`;
